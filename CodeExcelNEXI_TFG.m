%% --- SCRIPT PARA CREAR EXCEL FINAL: HIPOCAMPO 2 Y CUERPO CALLOSO (NEXI) ---
clear all; close all;

% --- CONFIGURACIÓN DE RUTAS LOCALES (NUEVA CARPETA CON CC) ---
ruta_base_txt     = 'C:\Users\lolan\Downloads\Diffusion_times\Resultados_NEXI_CC';
ruta_excel_datos  = 'C:\Users\lolan\Downloads\Diffusion_times\Datos.xlsx';

% Guardamos el Excel con un nombre único para no pisar el anterior
ruta_salida_excel = fullfile(ruta_base_txt, 'all_NEXI_CC_bueno.xlsx');
...
fprintf('--- GENERANDO BASE DE DATOS FINAL NEXI CC: all_NEXI_CC_bueno.xlsx ---\n\n');
...
for i = 1:height(T)
    ...
    % DESCARTE DEL RATÓN 87
    if strcmp(nombre_raton, 'R87')
        fprintf('[SALTADO] %s (descartado del estudio)\n', nombre_raton);
        continue; 
    end
    
    % Nombre del archivo TXT de la nueva carpeta
    nombre_archivo_txt = sprintf('%s_NEXI_Data.txt', nombre_raton);
    ...
    if exist(ruta_completa_txt, 'file')
        ...
        % Crear tabla temporal asignando las columnas exactas de NEXI
        tabla_actual = table(mouse_v, sex_v, diet_v, data(:,1), data(:,2), data(:,3), ...
                             data(:,4), data(:,5), data(:,6), data(:,7), ...
                             'VariableNames', {'MouseID','Sex','Diet','ROI','X','Y','di','de','f','t_ex'});
        ...
        fprintf('[!] No se encontró el archivo de datos en la carpeta CC para: %s\n', nombre_raton);
    end
end
...
if ~isempty(agrupacion_TOTAL)
    try
        ...
        fprintf('\n[ERROR] No se pudo guardar el Excel. ¡Cierra el archivo "all_NEXI_CC_bueno.xlsx" si lo tienes abierto!\n');
    end
else
    fprintf('\n[ERROR] No hay datos procesados en la carpeta CC para exportar.\n');
end